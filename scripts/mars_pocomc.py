...
R_core), constrained by mass / mean MoI / k2 / core_radius_km (see
``pylov3d.mars_mc``, "Identifiability" -- the 4th constraint is required
for a well-posed 4-parameter fit). Headless (matplotlib Agg backend, set
before any pyplot import). Saves the chain (samples + weights, .npz) and a
hand-rolled pairplot (no ``corner`` dependency), and prints posterior
medians +/- 1 sigma (3 significant figures) plus the effective sample size
(ESS), alongside the TASK-011 deterministic point fit for comparison.

Usage
-----
    venvLOV3Dconv/bin/python scripts/mars_pocomc.py --quick
    venvLOV3Dconv/bin/python scripts/mars_pocomc.py --n-particles 256

``--quick`` is a tiny smoke run (n_particles=32, a coarse solver grid,
...
# Run from the repo root with `venvLOV3Dconv/bin/python scripts/mars_pocomc.py`;
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="pocomc n_active (default: 256, or 16 with --quick).",
    )
    p.add_argument(
...
def run(args: argparse.Namespace) -> dict:
    ...
    if args.quick:
        ...
        # (measured: n_active=16 completes in ~20-60s at Nrbase=15).
    ...
    if args.quick:
        print(
            ...
            "posterior (measured: --quick intervals run ~2x too narrow versus an\n"
            "exact reference posterior -- e.g. propagating just the mass+moi_mean\n"
            "constraints analytically already implies sigma(rho_core) ~= 90 kg/m^3,\n"
            "so any converged marginal must be >= that). Use the (much slower)\n"
            "default settings for a result meant to be interpreted quantitatively."
        )
    print(f"\n{'param':<10} {'median':>14} {'-1sigma':>10} {'+1sigma':>10} {'TASK-011 point fit':>20}")
    for i, name in enumerate(MARS_FREE_PARAMS):
        med_r = _round_sig(medians[i])
        lo_r = _round_sig(medians[i] - lo1[i])
        hi_r = _round_sig(hi1[i] - medians[i])
        print(f"{name:<10} {med_r:>14.6g} {lo_r:>10.6g} {hi_r:>10.6g} {point_fit[i]:>20.6g}")

    model = build_model(MARS_PARAMETERIZATION, medians)
    obs = compute_observables(
        model, forcing, numerics, which=("mass", "moi_mean", "core_radius_km", "k2"),
    )
    print(
    ...
    npz_path = args.out_dir / ("mars_chain_quick.npz" if args.quick else "mars_chain.npz")
    np.savez(
    ...
    fig_path = args.out_dir / ("mars_pairplot_quick.png" if args.quick else "mars_pairplot.png")
    _pairplot(samples, weights, MARS_FREE_PARAMS, point_fit, fig_path)
    print(f"[output] pairplot saved to {fig_path}")

    return {
        "medians": medians, "lo1": lo1, "hi1": hi1, "point_fit": point_fit,
...
def _pairplot(samples: np.ndarray, weights: np.ndarray, names, truth, out_path: Path) -> None:
    ...
    weighted 1-D histogram, off-diagonal = weight-alpha scatter, truth marked
    with a red star/lines."""
    d = samples.shape[1]
    ...
    fig.suptitle("Mars posterior (pocomc) vs. TASK-011 point fit (dashed red)")
    fig.tight_layout()
