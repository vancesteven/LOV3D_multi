...
Raw-grid transform floor
------------------------
The MATLAB raw-grid SH transform leaves the +/-m rheology coefficients slightly
asymmetric even though the underlying physical field is real. After correcting
the Python scipy-to-LOV3D normalization, m=0 rheology coefficients agree at
~1e-9 relative while the +/-2 coefficients differ from the two MATLAB values by
~0.789%. Python enforces the exact real-field conjugacy relation instead. The
raw-grid Gate C therefore uses a 1% end-to-end lateral-response tolerance rather
than a solver-parity tolerance. Strict solver parity must be assessed on an
identical coefficient field, not by forcing Python to reproduce MATLAB's finite-
grid asymmetry.
"""
...
RAW_GRID_LATERAL_RESPONSE_TOL = 1.0e-2
...
def main() -> int:
    ...
    parser.add_argument(
        "--assert-matlab",
        action="store_true",
        help="at Nrbase=50 assert raw-grid MATLAB Gate C agreement within the documented transform floor",
    )
    ...
    if args.nrbase == 50:
        print("\nAuthoritative raw-grid MATLAB Gate C comparison:")
        for f, ku, kl in zip(forcings, k_u, k_l):
            print(
        ...
        if args.assert_matlab:
            ...
            for f, x in zip(forcings, k_l):
                assert relerr(x, matlab_lateral_k_ref(f.m)) < RAW_GRID_LATERAL_RESPONSE_TOL
            assert relerr(e_direct_u, MATLAB["e_direct_uni"]) < 5e-3
            assert relerr(e_direct_l, MATLAB["e_direct_lat"]) < RAW_GRID_LATERAL_RESPONSE_TOL
            assert relerr(e_love_u, MATLAB["e_love_uni"]) < 5e-3
            assert relerr(e_love_l, MATLAB["e_love_lat"]) < RAW_GRID_LATERAL_RESPONSE_TOL
            assert mismatch_u < 0.03
            assert mismatch_l < 0.03
            print("MATLAB raw-grid Gate C assertions: PASS (within documented raw-grid transform floor)")
