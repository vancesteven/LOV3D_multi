...
| TASK-021b | **B: hydration-amplitude sweeps** — run `get_love_hydrated` over the f_h × serpentinite-bracket grid at higher radial resolution (Nrbase≥50) + lmax=4, confirm A's Δk2 curve + ~65:1 mean:lateral ratio on independent hardware, optionally extend to lmax=6 to check (4,0)↔(2,0) channel convergence. Compute-heavy, token-light. Entry: `pylov3d/mars_hydration.py` docstring + `docs/MARS_MODEL.md` §5 | free | QUEUED — claimable now (all deps committed `370234d`) | — |
