...
| TASK-012 | **M6:** BODY-AGNOSTIC forward-model + MC framework (user roadmap: Mars → Moon → other bodies incl. gas giants later; Perple_X/PlanetProfile feed stage 1 eventually via compat.py). Three separable stages (θ→InteriorModel, model→observables, observables→likelihood), pocoMC sampler over 4 free Mars params, generic SH→lat/lon mapping utility + Mars fit-demonstration surface map. Lateral-field mapping (dichotomy/Tharsis) folds in after 013 data | A | IN-PROGRESS (impl running) | — |
| TASK-013 | **M6:** Mars SH data loaders — parse PDS gravity (MRO120D-class `.sha`) + MOLA topography coefficient files into `data/mars/`, with normalization checks (e.g. C20 vs published) and pinned checksums. **Codex candidate**; precondition: A fetches + documents the data files first (Codex runs offline) | free | QUEUED — spec after data landed | — |
| TASK-014 | **M6:** Machine B — part 1 (NOW, independent of 012/013): MATLAB LOV3D cross-check of the Mars 1D model in `docs/MARS_MODEL.md` (same 4 layers/densities/mu; confirm k2=0.169, h2=0.3156, l2=0.0516 — gives Mars the same MATLAB anchor the Moon has, and adjudicates the h2/k2=1.87 open item). Part 2 (after 012): heavy coupled Mars lateral spectra sweeps + MATLAB coupled cross-validation | B | QUEUED — part 1 claimable immediately | — |
