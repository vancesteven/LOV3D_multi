...
| TASK-014 pt 2 | **M6:** B — MATLAB coupled cross-check of the Mars lateral model. Everything needed is committed: `data/mars/mars_mu_variable_lateral.npz` (exact 23 amplitudes + provenance; MATLAB layer index = python + 1; eta0 must be OMITTED for elastic layers, NaN poisons the solve). Compare native LOV3D coupled spectrum vs `docs/MARS_MODEL.md` §"Lateral variations" table; k2 shift target +5.52e-5 | B | **CLAIMABLE NOW** | npz + MARS_MODEL.md |
| — | Suite runtime note: full suite now ~16 min (Mars lateral coupled solves). Candidate: move the heaviest to `@pytest.mark.slow` + addopts deselect with a CI-full lane | — | logged | — |
