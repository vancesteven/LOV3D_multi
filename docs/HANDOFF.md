...
| TASK-022 | **Codex: per-file Apache-2.0/SPDX headers** across pylov3d + Python scripts, README test-count refresh (502/516/38). Mechanical, no network | CODEX | QUEUED — spec ready, launch when convenient | `docs/tasks/TASK-022-codex-license-headers.md` |
| TASK-024 | **M8 prep: fetch Moon SH field data** (GRAIL SHADR gravity + MoonTopo2600p shape, mirroring data/mars practice: SOURCES.md, sha256 pins, loader verification) | A | IN-PROGRESS (Sonnet agent, 2026-08-08) | — |
| TASK-023 | **Codex: Moon SH data loaders + tests** (mirror TASK-013). BLOCKED until TASK-024 lands files; spec will be written against exact filenames | free | BLOCKED on 024 | spec TBD |
| TASK-025a | **Anelasticity stage a:** validate the viscoelastic 1D solver path against PyALMA3 (Maxwell + Andrade if supported), anelastic complex-k2 forwards for as-built Mars (verified forcing period) + Weber Moon (monthly tide); headline: does closing the Moon's 4.6% elastic k2 gap imply Q consistent with 38±4? Literature prior ranges for stage b | A | IN-PROGRESS (Sonnet build agent, 2026-08-08; Opus review to follow) | — |
