...
from .propagator import build_aprop, compute_gravity, CK_A, CK_B, CK_C
from .boundary_conditions import assemble_bc_no_ocean, assemble_bc_ocean
...
def get_solution(model, forcing, numerics):
    ...
    Y_all : (Nr+1, 8, 8) complex
        Fundamental matrix at each point (for energy calculations).
    Aprop_aux : (Nr+1, 3, 8) complex
        First 3 rows of Aprop at each point (for stress/strain recovery).
    """
